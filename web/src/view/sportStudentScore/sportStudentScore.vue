<template>
  <div>
    ...
    >
      ...
      <el-table-column label="大类" prop="class" width="120" /> 
      <el-table-column label="方法名称" prop="name" width="120" /> 
      <el-table-column label="分数" prop="score" width="120" /> 
      <el-table-column label="学校ID" prop="sportSchoolID" width="120" /> 
      <el-table-column label="学生ID" prop="sportStudentID" width="120" /> 
      <el-table-column label="测试数据" prop="testdata" width="120" /> <el-table-column label="按钮组">
        <template slot-scope="scope">
          <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="updateSportStudentScore(scope.row)">变更</el-button>
...
import {
  ...
  deleteSportStudentScore,
  deleteSportStudentScoreByIds,
  updateSportStudentScore,
  findSportStudentScore,
  getSportStudentScoreList
...
export default {
  ...
  data() {
    return {
      listApi: getSportStudentScoreList,
  ...
  // 条件搜索前端看此方法
    ...
    deleteRow(row) {
      this.$confirm('确定要删除吗?', '提示', {
        ...
        this.deleteSportStudentScore(row)
      })
    },
    async onDelete() {
      const ids = []
      ...
      const res = await deleteSportStudentScoreByIds({ ids })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
    ...
    async updateSportStudentScore(row) {
      const res = await findSportStudentScore({ ID: row.ID })
      this.type = 'update'
      if (res.code === 0) {
    ...
    async deleteSportStudentScore(row) {
      const res = await deleteSportStudentScore({ ID: row.ID })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
